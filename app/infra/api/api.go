...
const InfraURLEnv = `INFRA_URL`

func GetInfraURLFromEnv() string {
	return os.Getenv(InfraURLEnv)
}

type InfraAPI interface {
	// Load config by the given service name.
	LoadConfig(ctx context.Context, name string) (*LoadConfigResp, error)
}
...
// Load config by the given service name.
func (api *API) LoadConfig(ctx context.Context, name string) (out *LoadConfigResp, err error) {
	query := make(url.Values)
	query.Set("name", name)
	err = api.get(ctx, loadConfigEndpoint, query, &out)
	return
}
