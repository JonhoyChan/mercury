...
func (s *Service) CreateGroup(ctx context.Context, req *api.CreateGroupReq) (string, error) {
	...
	in := &persistence.GroupCreate{
		ClientID:     clientID,
		GroupID:      s.idGen.DecodeUid(uid),
		Name:         req.Name,
		GID:          uid.GID(),
		Introduction: req.Introduction,
		Owner:        s.idGen.DecodeUid(types.ParseUID(req.Owner)),
	}
...
func (s *Service) AddMember(ctx context.Context, req *api.AddMemberReq) error {
	...
	in := &persistence.GroupMember{
		ClientID: clientID,
		GroupID:  s.idGen.DecodeUid(types.ParseGID(req.GID)),
		UserID:   s.idGen.DecodeUid(types.ParseUID(req.UID)),
	}
