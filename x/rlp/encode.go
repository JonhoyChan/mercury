...
func (w *encbuf) encodeStringHeader(size int) {
	if size < 56 {
		...
		sizesize := putUint(w.sizebuf[1:], uint64(size))
...
// putUint writes i to the beginning of b in big endian byte
// order, using the least number of bytes needed to represent i.
func putUint(b []byte, i uint64) (size int) {
	switch {
	case i < (1 << 8):
		b[0] = byte(i)
		return 1
