...
func (h *HasherArgon2) Generate(p string) (string, error) {
	c := h.c.HasherArgon2()
	...
	// variant.
	hash := argon2.IDKey([]byte(p), salt, c.Iterations, c.Memory, c.Parallelism, c.KeyLength)
...
func (h *HasherArgon2) Compare(p1, p2 string) error {
	// Extract the parameters, salt and derived key from the encoded password
	// hash.
	c, salt, hash, err := decodeHash(p2)
	if err != nil {
		return err
	}

	// Derive the key from the other password using the same parameters.
	otherHash := argon2.IDKey([]byte(p1), salt, c.Iterations, c.Memory, c.Parallelism, c.KeyLength)
